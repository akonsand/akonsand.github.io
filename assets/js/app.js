...
main.controller('SearchController',['$rootScope','$scope','$http','ModalFactory','embedlyService','$stateParams','$location','wikiService','nlp',function($rootScope,$scope,$http,ModalFactory,embedlyService,$stateParams,$location,wikiService,nlp){
  var tweets = $("#tweets");
  ...
  function processNlp(searchInput) {
    var s= nlp.pos(searchInput).sentences[0];
    var p = s.nouns();
    if(p[0]) {
      console.log("reference to " + p[0].text);
      var word = p[0].text.capitalize();
      wikiService.get(word).then(function(success){
        var data = success.data;
        for(var page in data.query.pages){
          var pageData = data.query.pages[page];
          break;
        }
        $scope.wikiTitle = pageData.title;
        var content = pageData.extract.replace(/(<([^>]+)>)/ig,"");
        if(content.indexOf("redirect")!=-1){
          return;
        }
        if(content.length > 1500){
          $scope.wikiSummary = content.substring(0,1500) + "...";
        }
        else{
          $scope.wikiSummary = content;
        }
        $scope.showWiki = true;
      },function(failure){
        var a = failure;
...
main.factory('wikiService', function($http) {
  ...
  var wikiService = {
    get: function(searchterm) {
      return $http.jsonp('https://en.wikipedia.org/w/api.php?action=query&prop=extracts&format=json&exintro=&titles='+searchterm+'&callback=JSON_CALLBACK');
    }
  };
